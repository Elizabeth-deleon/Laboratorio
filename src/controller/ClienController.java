 ...
 * @author Grupo#3
 */
public class ClienController extends AbstractController{
	...
	cliente.getEmail()+"',"+ 
	cliente.getSaldo()+",'"+ 
	Helper.getFechaFormateada(cliente.getFechaIngreso(), Helper.AÑO_MES_DIA)+"')";
        try {
	...
	"FEC_ING = '"+Helper.getFechaFormateada(cliente.getFechaIngreso(), Helper.AÑO_MES_DIA)+"' WHERE "+
	"ID_CLI = '"+id+"' ";
       try {
            resultado = BaseConexion.getStatement().executeUpdate(sql);
        } catch (SQLException ex) {
